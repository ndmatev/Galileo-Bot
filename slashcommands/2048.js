...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (gameStates.has(userId)) {
      return interaction.reply({ content: 'You are already playing a game of 2048!', ephemeral: true });
    }
    ...
    const gameEmbed = await interaction.reply({
      embeds: [new EmbedBuilder()
        .setColor('#FFA500')
        .setTitle('🎮 2048 Game 🎮')
        .setDescription(`\`\`\`\n${formatBoard()}\n\`\`\``)
        .addFields({ name: 'Score', value: `${score}`, inline: true })
        .addFields({ name: 'Player', value: `${interaction.user.username}`, inline: true })
